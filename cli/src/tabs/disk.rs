...
// software is for
pub fn disk_tab(manager: &mut backend::Manager, scroll: u16) -> (Paragraph<'_>, u16) {
    let formatter = humansize::make_format(humansize::DECIMAL);
    #[allow(clippy::cast_possible_truncation)]
    let (paragraph, line_count) = manager.disk_information().map_or_else(
        || (Paragraph::new("No information available!"), 1),
        |disk_info| {
            let count = (disk_info.len() * 6) as u16;
            let text = disk_info
                .iter()
                .flat_map(|disk| {
                    vec![
